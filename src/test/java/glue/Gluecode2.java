...
public class Gluecode2
{
	...
	public void method6(String x,String y,String z)
	{
		...
		if(scfstep==scfres && ct.contains(y) && idfstep==idfres)
		{
			...
			//Reporter.log("Posts/Post retrieving/creation tests passed");
			Assert.assertTrue(true);
		}
		else
		{
			sh.s.write("Posts/Post retrieving/creation tests failed");
			//Reporter.log("Posts/Post retrieving/creation tests failed");
	...
	public void method9(DataTable dt)
	{
		...
		for(Map<String,String> m:l)
		{
			...
			if(cr.equals("unique") && statuscode==sc)
			{
				...
				//Reporter.log("Service is working correctly in new posts creation");
				Assert.assertTrue(true);
			}
			else
			{
				...
				//Reporter.log("Service is not working correctly in new posts creation as statuscode recieved from service is "+statuscode+" which is not "+sc");
				Assert.assertTrue(false);
			}
		}
